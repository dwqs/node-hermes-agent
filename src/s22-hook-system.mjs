...
import { gatewayHookRegistry, pluginHookRegistry } from './common/hook-system.mjs'
...
function runCli() {
  ...
  // s22
  gatewayHookRegistry.discoverAndLoad(process.env.HERMES_HOME)
  gatewayHookRegistry.emit('gateway:startup')

  pluginHookRegistry.registerHook('pre_tool_call', ({ name }) => {
    console.log(chalk.green(` [hook:pre_tool_call] 工具调用: ${name}  `))
  })
  pluginHookRegistry.registerHook('post_tool_call', (msg) => {
    console.log(chalk.green(` [hook:post_tool_call] 工具调用结果 === name: ${msg.name} ===id: ${msg.tool_call_id} `));
  })
...
async function runLegacyCli() {
  ...
  pluginHookRegistry.registerHook('post_tool_call', (msg) => {
    console.log(chalk.green(` [hook:post_tool_call] === name: ${msg.name} ===id: ${msg.tool_call_id} `));
  })
