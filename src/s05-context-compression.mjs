...
import { model, COMPRESSION_THRESHOLD, MAX_ITERATIONS } from './common/model.mjs'
...
import { estimateTokens, compress } from './common/context-compression.mjs'
...
async function runConversation(input, db, sessionId, systemPrompt) {
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if(estimateTokens(messages) > COMPRESSION_THRESHOLD) {
      ...
      console.log(chalk.green(`🔍 压缩成功，当前 token 数: ${estimateTokens(messages)}`))
      console.log(chalk.green(`🔍 压缩后的消息: ${messages.map(m => m.content).join('\n')}`))
    }
...
async function main() {
  ...
  console.log('=== s05: Context Compression ===')
  console.log(`模型名称: ${process.env.AI_MODEL_NAME}`, `压缩阈值: ${COMPRESSION_THRESHOLD}`)
